...
from app.models import (
    Criterion, Error, Explanation, NameMode, Ranking, RankingRow, Weights, WindowMode,
)
from app.ranking import (
    above_name, gap_to_below, gap_to_next, previous_ranks, rank_table,
)
...
def view_options(
    ...
    window: WindowMode = Query(default="week", description="`week` scores only that week; `cumulative` scores week 1 through `week`."),
    ...
    return ViewOptions(week, window, keys or None, parse_weights(weights), name_mode)
...
def resolve_view(ctx: AppContext, snapshot: Snapshot, options: ViewOptions) -> View:
    ...
    return View(options.week, options.window, selected, weights, options.name_mode)
...
def get_ranking(class_id: str, options: ViewOptions = Depends(view_options), ctx: AppContext = Depends(get_ctx)):
    ...
    rows = rank_table(
        snapshot, week_id=view.week_id, window=view.window,
        criteria=view.criteria, weights=view.weights, name_mode=view.name_mode,
    )
...
@router.get(
    ...
    summary="Per-criterion breakdown of one student's rank",
